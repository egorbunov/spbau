...
begin
    ...
    read(n);
    assert((min_n <= n) and (n <= max_n), 'N out of bounds');

    for i := 1 to n do begin
        read(x[i], y[i]);
        assert((0 <= x[i]) and (x[i] <= max_c), format('X[%d] out of bounds', [i]));
        assert((0 <= y[i]) and (y[i] <= max_c), format('Y[%d] out of bounds', [i]));
    end;
    x[n + 1] := x[1];
    y[n + 1] := y[1];
    ...
    repeat
        ...
        assert((abs(cx - nx) > eps) or (abs(cy - ny) > eps));
