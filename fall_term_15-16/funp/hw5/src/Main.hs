...
maybeP p = np <|> jp where
    jp = pure (\x y z -> Just z) <*> string "Just" <*> symbol ' ' <*> p
    np = pure (const Nothing) <*> string "Nothing"

-- 3. (0.5 балла)
listP :: Parser Char a -> Parser Char [a]
listP p = brackets $ sepBy p (symbol ',')

spacedp :: Parser Char a -> Parser Char a
spacedp p = between spaces spaces p

-- 4. (0.5 балла)
listP' :: Parser Char a -> Parser Char [a]
listP' p = brackets $ sepBy p (spacedp (symbol ','))
...
treeP nodeP leafP = parseLeaf <|> parseNode where
    parseLeaf = Leaf <$> leafP
    parseNode = angles $ Node <$> treeP nodeP leafP <*> braces nodeP <*> treeP nodeP leafP
