...
namespace {
    struct HuffmanTree {
        ...
        HuffmanTree(au::byte_t c, size_t cnt) : zeroChild(nullptr), oneChild(nullptr), byte(c), cnt(cnt), isUsed(false) {
        }
    ...
    au::Result<int> dumpTreeAndEncode(FILE* in, FILE* out, HuffmanTree* tree) {
        ...
        // TODO: delete that debug info below
        std::cout << "INITIAL LEN IN BYTES = " << tree->cnt << std::endl;
        std::cout << "FINAL LEN IN BYTES = " << finalByteNum << std::endl;
        std::cout << "NOT USED BITS AT END = " << (int) notUsedAtEnd << std::endl;
        std::cout << "FINAL LEN IN BITS = " << finalLenInBits << std::endl;
        std::cout << "NUMBER OF BYTES = " << (int) byteNum << std::endl;
        ...
        while (true) {
            ...
            for (int i = 0; i < curCode.getSize(); ++i) {
                bitWriter.write(curCode[i]);
            }
        }
        bitWriter.flush();

        std::cout << "COMPRESSED FILE LENGTH = " << finalFileSize << std::endl;
        std::cout << "COMPRESS RATIO = " << (double) finalFileSize / tree->cnt << std::endl;
...
//            std::cout << (int) descriptor << " ; ";
//            std::cout << byteNum << " ; ";
//            std::cout << unusedBitNum << " ; ";
//            std::cout << byteNum * au::BITS_IN_BYTE - unusedBitNum << "; ";
            std::cout << byte << " : ";
